...
public class HW3 {
  ...
  public static void main(String[] args) {
    if (args.length != 4) {
     System.out.println("usage: java HW3 <modeFlag> <trainFilename> " + "<tuneFilename> <testFilename>");
     System.exit(-1);
    }

    /*
     * mode 0 : output the INFORMATION GAIN of each attribute at the root node 1 : create a
     ...
     */
    int mode = Integer.parseInt(args[0]);  //what mode we choose
    if (0 > mode || mode >= 5) {
      System.out.println("mode must be between 0 and 4");
      System.exit(-1);
    }

    //outputting INFORMATION GAIN
    if (mode == 0) {
    ...
    if (mode > 2) {
      //creates identical tuneSet
      tuneSet = createDataSet(args[2]);
      //checks to make sure meta values are same in both
